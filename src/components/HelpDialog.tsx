...
const SECTIONS: Array<{ title: string; body: string[] }> = [
  ...
  },
  {
    ...
    body: [
      ...
      "IMAGE PATH: a zip of every object for any external image-to-3D tool. \"Download image pack (.zip)\" is free and instant — the pipeline's cutout (+ Zero123++ side views if you ran them), each normalised to a 1024px white canvas. \"AI turnaround\" (paid) re-renders all five views per object — perspective + front/back/left/right — large and clean on white via an image model (Gemini or OpenAI key); perspective/front are observed, back/sides are inferred. Both are on the confirm block and on finished jobs; neither touches Tripo/Meshy.",
      ...
      "ASSET LIBRARY (free): the confirm block also has an \"Asset library\" section — per object, \"Find a free asset\" pulls ranked CC0 matches from Poly Haven; pick one and it downloads into assets/library/ and attaches like a generated model (and becomes the preferred one in scene.json). A CREDITS.txt is written automatically. \"Use N library assets & finish\" completes a job with no provider spend. Best for generic furniture/props; mix it with AI gen for the bespoke pieces.",
    ],
  },
  {
